window.onload = function () {
    ...
    fetch("http://localhost:3000/receipt/1")
        ...
        .then((data) => {
            ...
            document.getElementById("time2").innerText = `${data.date.hour}:${data.date.minute}   ${data.date.day}/${data.date.month + 1}/${data.date.year} `
            var billDate = new Date(data.date.year, data.date.month, data.date.day, data.date.hour, data.date.minute);
            ...
            document.getElementById("price").innerText = Math.round(data.amount*diffInHours.toFixed(1));
            // document.getElementById("time2").innerText = `${data.date.hour}:${data.date.minute}   ${data.date.day}/${data.date.month}/${data.date.year} `
        });
